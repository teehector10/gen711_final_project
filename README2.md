...
## Findings
When conducting our phylogenic tree, we were able to visualize the ancestral background of differing COVID-19 isolations. We were not able to add the original Wuhan strain to this tree because of constraints with the NCBI website feature. We also limited the tree to ten samples in order to create a clearer picture for the relation between a variety of isolations. Seen in Figure 4, there is an obvious differenence in phylogeny from 2020 samples to 2022 samples. With the possibility of there being three strains, 2022 data exhibits branching from the original ancestral genome as opposed to those of the 2020 origin. 
...
After aligning the sequences in NCBI, we were able to look at how the COVID-19 genome differed in nucleotides sequences. The red color at the top of Figure 5 shows areas of mutations. There is obvious variation between the 2020 and 2022 samples, with the addition of an outlier sample in 2022, it can be observed that there are more mutations in comparion in the 2022 samples.
...
Using a program called SnapGene, we were able to align our sequences and compare them to the original sequence from Wuhan and look for changes. Our goal with using this program was to look for differences, but we decided to go further and try to discover at where the most mutations occured. The spike protein can be defined as a known area of mutations between strains, and is located at 21,500 bp and 25,300 bp (PubMed). So by analyzing this alignment we are able to see that there are mutations where this spike protein is located that are present in the 2022 sequences and not the 2020, showing us how COVID has mutated over time. 
...
By using SnapGene we were able to map out the domains of the spike protein based off of Figure 3. Figure 3 was an amino acid sequence so we had to convert our alignment to a nucleotide sequence. We were able to do this by multiplying the amino acid position by three. But what is really interesting about Figure 6 is that in the Receptor-Binding Domain for the 2022 sequences (ON345288.1 - ON345302.1) there were a multitude of mutations that did not exist in 2020. These mutations within the RBD domain region give reason to believe there are changes in the way that the spike protein binds to a human cell. The ACE2 receptor is a protein on the surface of a human cell that is known to be where the spike protein binds. Whether or not these mutations decrease or increase the likelyhood of these two regions binding is unable to be interpreted through this alignment. However, the alignment is able to show that the way COVID-19 has mutated has affected this region of its genome compared to sequences from 2020.  
